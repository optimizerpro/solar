...
class Contracts_model extends App_Model
{
    ...
    public function send_contract_for_adobe_sign($id,$code){
        ...
        $form = (array)$contract;
        $doc_array=array();
        if(!is_dir(CONTRACTS_UPLOADS_FOLDER.$id.'/')){
            mkdir(CONTRACTS_UPLOADS_FOLDER.$id.'/');
            mkdir(CONTRACTS_UPLOADS_FOLDER.$id.'/agreements/');
        }
        if(!is_dir(CONTRACTS_UPLOADS_FOLDER.$id.'/agreements/')){
            mkdir(CONTRACTS_UPLOADS_FOLDER.$id.'/agreements/');
        }
        ...
        if(!empty($contact)){
            ...
            $staff=(array)get_staff(get_staff_user_id());
            $email2=$staff['email'];
            $docs=signJoineeDocuments($code,$contact['email'],$doc_array,$id,$email2);
            if($docs==false){
                return false;
            }
            else
